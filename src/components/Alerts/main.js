  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const formattedAlerts = alerts.map(alert => {
        ...
        const filteredCommonIssues = {};
        if (alert.commonIssues) {
          Object.keys(alert.commonIssues).forEach(site => {
            if (allowedSites.includes(site)) {
              filteredCommonIssues[site] = alert.commonIssues[site];
            }
          });
        }

        // Filter Hourly Results
        const filteredHourlyResults = (alert.filteredHourlyResults || []).map(hourly => {
          const filteredErrorCounts = {};
          if (hourly.filteredErrorCounts) {
            Object.keys(hourly.filteredErrorCounts).forEach(site => {
              if (allowedSites.includes(site)) {
                filteredErrorCounts[site] = hourly.filteredErrorCounts[site];
              }
            });
          }
          return {
        ...
        }).filter(hourly => Object.keys(hourly.filteredErrorCounts).length > 0);

        return {
          ...alert,
          date: formatDateToDDMMYYYY(alert.date || date),
          commonIssues: filteredCommonIssues,
          filteredHourlyResults: filteredHourlyResults
        };
      }).filter(alert => 
        Object.keys(alert.commonIssues).length > 0 || 
        alert.filteredHourlyResults.length > 0
      );
